...
switch ($path) {
        ...
        switch ($object) {
                    ...
                    case 'POST':
                        ...
                        if (empty($errors)) {
                            ...
                            if (
                                ...
                                $send_email = Email::send_client_welcome_email(
                                    $email_data
                                );
                            }
            ...
            case 'system':
                if (APP_STAGE == 'Demo') {
                    jsonResponse([
                        'error' => true,
                        'message' => 'Not available in the demo!',
                    ]);
                }
                    ...
                    case 'POST':
                        ...
                        switch ($cmd) {
                            ...
                            case 'backup_database':
                                ...
                                $directory = 'storage/' . $taskId;
                                $path = $directory . '/db.sql';

                                if (!file_exists($directory)) {
                                    sp_create_directory($directory);
                                    sp_create_index_file_to_prevent_directory_listing(
                                        $directory
                                    );
                                }
                            ...
                            case 'download_file':
                                ...
                                $response = (new Http())->get($file_url);
    ...
    case 'client-auth':
        ...
        if ($validation->fails()) {
            ...
            if ($client) {
                if (
                    Password::_verify($data['password'], $client->password) ==
                    true
                ) {
                    if ($client->autologin == '') {
                        $token = Misc::random_string(20) . $id . time();
                    ...
                    case 'GET':
                        ...
                        foreach ($invoices as $invoice) {
                            $recent_invoices[] = [
                                ...
                                'issued_date_formatted' => date(
                                    $config['df'],
                                    strtotime($invoice->date)
                                ),
                                'due_date' => $invoice->due_date,
                                'due_date_formatted' => date(
                                    $config['df'],
                                    strtotime($invoice->duedate)
                                ),
                    ...
                    case 'GET':
                        ...
                        foreach ($quotes as $quote) {
                            $quotes_data[] = [
                                ...
                                'issued_date_formatted' => date(
                                    $config['df'],
                                    strtotime($quote->datecreated)
                                ),
                                'valid_until_date_formatted' => date(
                                    $config['df'],
                                    strtotime($quote->validuntil)
                                ),
                                'status' => $quote->status,
                                'preview_url' =>
                                    U .
                ...
                switch ($method) {
                    case 'GET':
                        ...
                        foreach ($items as $item) {
                            ...
                            if ($item->image != '') {
                                $image =
                                    APP_URL . '/storage/items/' . $item->image;
                            }

                            if ($item->image_thumbnail != '') {
                                $image_thumbnail =
                                    APP_URL .
                                    '/storage/items/thumb_' .
                                    $item->image;
                            }
