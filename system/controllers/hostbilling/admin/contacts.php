...
switch ($action) {
    ...
    case 'view':
        ...
        if ($contact) {
            ...
            $quotes_count = Quote::where('userid', $contact->id)->count();

            $ui->assign('tickets_count', $tickets_count);
            $ui->assign('invoices_count', $invoices_count);
            $ui->assign('quotes_count', $quotes_count);

            switch ($tab) {
                case 'tickets':
                    $tickets = Ticket::where('userid', $contact->id)
                        ->orderBy('id', 'desc')
                        ->get();
                    \view('hostbilling/admin/contact_tickets', [
                        'tickets' => $tickets,
                    ]);
                    break;
                case 'invoices':
                    $invoices = Invoice::where('userid', $contact->id)
                        ->orderBy('id', 'desc')
                        ->get();
                    \view('hostbilling/admin/contact_invoices', [
                        'invoices' => $invoices,
                    ]);
                    break;
                case 'quotes':
                    $quotes = Quote::where('userid', $contact->id)
                        ->orderBy('id', 'desc')
                        ->get();

                    \view('hostbilling/admin/contact_quotes', [
                        'quotes' => $quotes,
                    ]);
                ...
                case 'orders':
                    $orders = HostingOrder::where('contact_id', $contact->id)
                        ->orderBy('id', 'desc')
                        ->get();
                    \view('hostbilling/admin/contact_orders', [
                        'orders' => $orders,
                    ]);
                    break;
                case 'domains':
                    $domains = DomainOrder::where('contact_id', $contact->id)
                        ->orderBy('id', 'desc')
                        ->get();

                    \view('hostbilling/admin/contact_domains', [
                        'domains' => $domains,
                    ]);
                    break;
                ...
                case 'edit':
                    $fs = ORM::for_table('crm_customfields')
                        ...
                        ->find_many();
                    $ui->assign('fs', $fs);
                    $ui->assign('countries', Countries::all($contact->country));
                    $tags = Tags::get_all('Contacts');
                    $ui->assign('tags', $tags);
                    $dtags = explode(',', $contact->tags);
                    $ui->assign('dtags', $dtags);
                    ...
                    \view('hostbilling/admin/contact_edit', [
                        'owners' => $owners,
                    ]);
            ...
            if (
                ...
                if ($email_config) {
                    ...
                    if ($e) {
                        ...
                        $message->set('client_name', $account);
                        $message->set('client_email', $email);

                        $message->set('client_password', $u_password);
                        ...
                        $message_email = (new Swift_Message($subj))
                            ->setFrom([
                                $config['sysEmail'] => $config['CompanyName'],
                            ])
                            ->setTo([$email => $account])
                            ->setBody($message_o, 'text/html');

                        $mailer->send($message_email);
                    }
                }
            }
    ...
    case 'find_by_group':
        ...
        if ($gid) {
            ...
            if ($g) {
                ...
                $ui->assign(
                    'xfooter',
                    Asset::js(['js/redirect', 'js/group_email'])
                );
...
_L[\'are_you_sure\'] = \'' .
 ...
 '
        ...
        e.preventDefault();
        $.redirect(base_url + "handler/bulk_email/",{ type: "customers", ids: ' .
                        json_encode($ids) .
                        '});
    });
...
<table class="table table-bordered">
   ...
   <tbody>
    ...
    case 'drive':
        ...
        $ui->assign('xheader', Asset::css(['footable/css/footable.core.min']));
        $ui->assign('xfooter', Asset::js(['footable/js/footable.all.min']));
