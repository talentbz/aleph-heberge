...
class Backup
{
    ...
    {
        $filesystems = new Filesystems\FilesystemProvider(
            Config::fromPhpFile('system/helpers/backupfiles.php')
        );
        ...
        $databases = new Databases\DatabaseProvider(
            Config::fromPhpFile('system/helpers/backupdb.php')
        );
    ...
    public function backupDatabaseMySqli(
        $directory,
        $outname,
        $action = 'store'
    ) {
        ...
        if (!function_exists('mysqli_connect')) {
            die(
                ' This scripts need mysql extension to be running properly ! please resolve!!'
            );
        }
        ...
        $dir = $directory;
        $result =
            '<p> Could not create backup directory on :' .
            $dir .
            ' Please Please make sure you have set Directory on 755 or 777 for a while.</p>';
        $res = true;
        if (!is_dir($dir)) {
            if (!@mkdir($dir, 755)) {
                $res = false;
        ...
        if ($res) {
            ...
            if (file_exists($dir . '/' . $name . '.sql.gz')) {
                for (
                    $i = 1;
                    @count(file($dir . '/' . $name . '_' . $i . '.sql.gz'));
                    $i++
                ) {
                    $name = $name;
                    if (
                        !file_exists($dir . '/' . $name . '_' . $i . '.sql.gz')
                    ) {
                        $name = $name . '_' . $i;
                        break;
                    }
