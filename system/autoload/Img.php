...
class Img
{
            ...
            if ($this->_create_from_source()) {
                ...
                if (is_array($filter)) {
                    ...
                    foreach ($filter as $arguments) {
                        // if filter exists
                        if (
                            defined('IMG_FILTER_' . strtoupper($arguments[0]))
                        ) {
        ...
        if ($this->_create_from_source()) {
            ...
            if (
                ...
                if ($width == 0 && $height > 0) {
                    ...
                    if (
                        round(
                            $horizontal_aspect_ratio * $this->source_height <
                                $height
                        )
                    ) {
                        // the target image's width is as given as argument to the method
                        ...
                        $target_width = round(
                            $vertical_aspect_ratio * $this->source_width
                        );
                    }
                    ...
                    // compute the target image's height, preserving the aspect ratio
                    $target_height = round(
                        $this->source_height / $aspect_ratio
                    );
                    // for any other case
                } else {
            ...
            if (
                ...
                ($width > 0 && $height > 0
                    ...
                    : $this->source_width > $target_width ||
                        $this->source_height > $target_height)
            ) {
                // if
                if (
    ...
    public function rotate($angle, $background_color = -1)
    {
        ...
        if ($this->_create_from_source()) {
            ...
            if (
                ...
                $this->source_type == IMAGETYPE_GIF &&
                ...
                // rotate the image
                $this->source_identifier = imagerotate(
                    $this->source_identifier,
                    $angle,
                    $background_color
                );
    ...
    private function _create_from_source()
    {
        ...
        if (!function_exists('gd_info')) {
            ...
            $this->target_type = strtolower(
                substr($this->target_path, strrpos($this->target_path, '.') + 1)
            );
            ...
            switch ($this->source_type) {
                ...
                case IMAGETYPE_GIF:
                    ...
                    if (
                        ($this->source_transparent_color_index = imagecolortransparent(
                            $identifier
                        )) >= 0
                    ) {
    ...
    private function _hex2rgb($color, $default_on_error = '#FFFFFF')
    {
        ...
        if (
            preg_match('/^#?([a-f]|[0-9]){3}(([a-f]|[0-9]){3})?$/i', $color) ==
            0
        ) {
     ...
     *  @access private
     */
    private function _prepare_image(
        $width,
        $height,
        $background_color = '#FFFFFF'
    ) {
        // create a blank image
        $identifier = imagecreatetruecolor(
            (int) $width <= 0 ? 1 : (int) $width,
            (int) $height <= 0 ? 1 : (int) $height
        );
    ...
    private function _sharpen_image($image)
    {
        ...
        if (
            ...
            $matrix = [[-1.2, -1, -1.2], [-1, 20, -1], [-1.2, -1, -1.2]];
    ...
    private function _write_image($identifier)
    {
        ...
        if (
            $disabled_functions == '' ||
            strpos('chmod', $disabled_functions) === false
        ) {
            // chmod the file
